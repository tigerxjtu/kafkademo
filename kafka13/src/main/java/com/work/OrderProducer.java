...
import com.work.serdes.GenericSerializer;
...
public class OrderProducer {
	...
	public static void main(String[] args) throws Exception {
		...
		orders.forEach((Order order) -> producer.send(new ProducerRecord<String, Order>("orders2", order.getUserName(), order)));
		producer.close();
	}
	...
	public static List<Order> readOrder() throws IOException {
		...
		List<Order> orders = lines.stream()
			...
			.map((String[] values) ->{ 
				Order order=new Order(values[0], values[1], LocalDateTime.parse(values[2], dataTimeFormatter).toEpochSecond(ZoneOffset.UTC) * 1000, Integer.parseInt(values[3]));
